...
26. Создайте индекс для оптимизации суффиксного поиска:
    ...
    *Объясните результаты:*
    ...
    Второй вариант должен ускорить поиск,так как используется триграммный индекс, который сразу находит строки, в которых есть все необходимые триграммы, и это исключает необходимость просматривать все строки
...
29. Выполните поиск по началу названия:
    ```sql
    EXPLAIN ANALYZE
    SELECT * FROM t_books WHERE title ILIKE 'Relational%';
    ```
