...
20. Выполните запрос из шага 13 с использованием UPPER:
    ...
    *Объясните результат:*
    Скорость выполнения выросла, так как мы привели всё к одному регистру
...
22. Выполните поиск подстроки:
    ...
    *Объясните результат:*
    Так как выполнен поиск совпадений не с начала строки, BTree-индекс не используется
...
25. Создайте индекс для оптимизации суффиксного поиска:
    ```sql
    -- Вариант 1: с reverse()
...
27. Выполните поиск по началу названия:
    ```sql
    EXPLAIN ANALYZE
    SELECT * FROM t_books WHERE title ILIKE 'Relational%';
    ```
...
28. Создайте свой пример индекса с обратной сортировкой:
    ```sql
